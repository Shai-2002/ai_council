...
export default function RoleHistoryPage() {
  ...
  if (!role) {
    return <div className="flex-1 flex flex-col items-center justify-center h-full text-zinc-500">Role not found.</div>;
  }
    ...
    <div className="flex-1 flex flex-col h-full bg-zinc-50 dark:bg-zinc-950 overflow-y-auto">
      <div className="p-6 md:p-10 max-w-4xl mx-auto w-full space-y-6">
        ...
        <div className="space-y-4">
          {loading ? (
            <div className="text-center py-16 text-zinc-400">Loading chats...</div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-4 text-xs text-zinc-500 dark:text-zinc-400">
                  <span className="flex items-center gap-1.5 font-medium">
                    ...
                    {new Date(chat.updated_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                  </span>
                  <span className="truncate">{chat.project_id ? 'In project' : 'Independent session'}</span>
                </div>
              </div>

              <div className="hidden sm:flex shrink-0">
                <ChevronRight className="h-5 w-5 text-zinc-300 dark:text-zinc-700 transition-colors" />
              </div>
            </Link>
          )) : (
            <div className="text-center py-16 px-4 rounded-3xl border-2 border-dashed border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/20">
              ...
              <p className="text-zinc-500 dark:text-zinc-400">You haven&apos;t started any conversations with the {role.title} yet.</p>
              <Button onClick={handleCreateChat} variant="outline" className="mt-6">
